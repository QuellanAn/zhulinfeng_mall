...
alter table cms_topic_comment comment '话题评论表';
...
(
   ...
   product_sku_id       bigint comment '商品货号',
   member_id            bigint,
...
(
   ...
   product_sn           varchar(64) comment '商品sn 编号',
...
(
   ...
   continue_weight      decimal(10,2) comment '超出首重kg',
   continme_fee         decimal(10,2) comment '超出首重计费',
...
(
   ...
   member_price         decimal(10,2) comment '该等级会员的商品价格',
...
(
   id                   bigint not null auto_increment,
   product_attribute_category_id bigint comment '品属性分类id',
   name                 varchar(64),
...
(
   id                   bigint not null auto_increment,
   parent_id            bigint comment '父级分类的编号：0表示一级分类',
   name                 varchar(64),
...
(
   id                   bigint not null auto_increment,
   product_category_id  bigint comment '产品分类iid ',
...
(
   id                   bigint not null auto_increment,
   product_id           bigint comment '商品ID',
   full_price           decimal(10,2) comment '原始价格',
   reduce_price         decimal(10,2) comment '减少的价格',
   primary key (id)
);
...
(
   ...
   price_new            decimal(10,2) comment '改变后的价格',
   sale_price_old       decimal(10,2) comment '改变前优惠价',
   sale_price_new       decimal(10,2) comment '改变后优惠价',
   gift_point_old       int comment '改变前赠送的积分',
   gift_point_new       int comment '改变后赠送的积分',
...
alter table pms_product_operate_log comment '商品操作记录表';
...
alter table pms_sku_stock comment '商品sku的库存';
...
(
   ...
   collect_product_count int comment '收藏商品数',
   collect_subject_count int comment '收藏专栏数',
   collect_topic_count  int comment '收藏主题数',
   collect_comment_count int comment '收藏评论数',
   invite_friend_count  int comment '邀请好友数',
